...
def active_users(cr, active_window_minutes=30):
    ...
    Source is bus.presence, verified against
    addons/bus/models/bus_presence.py on the 18.0 branch. (Odoo 19 renames it to
    mail.presence / mail_presence; on 18.0 the table is bus_presence.) It is a
    one-row-per-user table with a unique index on user_id, so this is cheap
    ...
    if not _table_exists(cr, "bus_presence"):
