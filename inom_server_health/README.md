...
| Requirement | Why |
|---|---|
| Odoo 19.0 Community or Enterprise | Target release. |
...
`dev-tools/verify_views.py` validates every view arch against Odoo 19's own
RNG schemas (vendored from `odoo/addons/base/rng/` on the 19.0 branch). Run it
before packaging:
...
It exists because v19.0.4.0.0 shipped a search view using
`<group expand="0" string="Group By">`. Odoo 19 allows `<group>` neither
attribute and requires its children to be `<field>`, so the upgrade failed
with a bare "Invalid view definition". The schema says exactly what is wrong;
guessing does not.

Note: Odoo 19 ships no RNG for form views — those are validated in Python at
...
That is not a style choice. In Odoo 19 a stored-field write does not issue
SQL: `Field.write` updates the cache and marks the record dirty, and `_flush`
