...
| Requirement | Why |
|---|---|
| Odoo 17.0 Community or Enterprise | Target release. |
...
`dev-tools/verify_views.py` validates every view arch against Odoo 17.0's own
RNG schemas (vendored from `odoo/addons/base/rng/` on the 17.0 branch). Run it
before packaging:
...
Note: Odoo 17.0 ships no RNG for form views — those are validated in Python at
...
That is not a style choice. In Odoo 17 a stored-field write does not issue
SQL either: `Field.write` updates the cache and queues the record in
`env.all.towrite`, and the flush groups the queued records by the values they
need before emitting UPDATEs. Assigning on the record and calling `write()`
therefore reach the database as the same statements, and the assignment form
drops a redundant `browse()` per group. (Odoo 18 replaced the flush path with
`_write_multi` and a single `UPDATE ... FROM (VALUES ...)` per distinct set of
columns; the conclusion here is unchanged, only the shape of the SQL.)
...
`hour_interval.js` reproduces that split on Odoo 17.0: it appends `hour` to
...
The same file also fills two gaps in `SampleServer` (`@web/model/sample_server`),
which draws placeholder data while the sample table is still empty. Its
`FORMATS` and `INTERVALS` maps are keyed by interval and contain no `hour`
entry on any current release -- Odoo 19 included -- so the fresh-install path
threw *"Cannot read properties of undefined (reading 'length')"* out of
`Formatter.parseFormat`. The label format added here is the one the ORM itself
uses (`READ_GROUP_DISPLAY_FORMAT['hour']`), so placeholder and real labels
match. A running instance with history never reaches this code.
