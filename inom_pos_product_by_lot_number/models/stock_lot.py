...
class StockLot(models.Model):
    ...
    def _iml_lot_fields(self):
        """Return the lot fields safe to read on this install.

        `expiration_date` only exists when the optional `product_expiry`
        module is installed. Requesting it otherwise makes search_read raise
        "Invalid field 'expiration_date'", which is what made the lot popup
        come back empty ("No lot/serial numbers available") on Odoo 18.
        """
        candidate = ['id', 'name', 'product_id', 'product_qty', 'ref', 'expiration_date']
        return [f for f in candidate if f in self._fields]

    @api.model
    def get_lots_by_product(self, product_id, limit=100):
        """Return available lots with real-time qty from stock.quant."""
        if not product_id or not isinstance(product_id, int):
            return []

        # stock.quant se real-time available qty lao
        ...
        # lot_id wise qty group karo
        lot_qty = {}
        for q in quants:
            lid = q.lot_id.id
            lot_qty[lid] = lot_qty.get(lid, 0) + q.quantity

        if not lot_qty:
            return []

        lots = self.browse(lot_qty.keys())
        result = []
        for lot in lots:
            if lot.product_id.id != product_id:
                continue
            qty = lot_qty.get(lot.id, 0)
            if qty <= 0:
                continue
            rec = {
                'id': lot.id,
                ...
                'product_qty': qty,  # real-time qty
            }
            if 'expiration_date' in self._fields:
                rec['expiration_date'] = lot.expiration_date
            if 'ref' in self._fields:
                rec['ref'] = lot.ref
            result.append(rec)

        result.sort(key=lambda x: x['name'])
        return result[:max(1, min(int(limit or 100), 500))]
    ...
    #     ]
    #     fields = self._iml_lot_fields()
    #     order = ('expiration_date asc, name asc'
    #              if 'expiration_date' in self._fields else 'name asc')
    ...
    def create_lot_from_pos(self, vals):
        ...
        if existing:
            return {
                ...
                'expiration_date': (existing.expiration_date or False) if 'expiration_date' in existing._fields else False,
                'duplicate': True,
            }
        ...
        return {
            ...
            'expiration_date': (new_lot.expiration_date or False) if 'expiration_date' in new_lot._fields else False,
            'duplicate': False,
        }
