...
export class LotSelectionPopup extends Component {
    ...
    static props = {
        ...
        // Already-used qty per lot_name in the current order (Phase 4)
    ...
    setup() {
        ...
        onWillUnmount(() => {
            if (!this._resolved) {
                this.props.getPayload({ confirmed: false, lots: [], totalQty: 0 });
                this._resolved = true;
            }
        });
    }
    ...
    async confirm() {
        ...
        if (this.isSerial && this.checkDuplicateSerial) {
            const checks = [...this.state.selected.keys()]
                .map(n => this._validateSerialAsync(n));
        ...
        this.props.getPayload({
            confirmed: true,
            lots,
            totalQty: this.totalSelectedQty,
        });
        this.props.close();
    }
