...
class TestMoReset(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # Plain products (default 'consu'); components still generate raw moves
        # on a Manufacturing Order, which is all the reset logic needs. Avoids
        # version-specific storable fields (is_storable / detailed_type).
    ...
    def test_workorders_restored(self):
        ...
        mo.with_user(self.reset_user).action_set_to_draft()
        self.assertTrue(
            all(w.state in ('pending', 'waiting', 'ready') for w in mo.workorder_ids),
            "All work orders must be revived out of 'cancel' to an active state.")
        self.assertFalse(any(w.state == 'cancel' for w in mo.workorder_ids))
