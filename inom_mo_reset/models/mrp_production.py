...
# Work order states that represent a "finished or aborted" operation. These are
# the states we want to revive when an MO is reset to draft.
_WO_INACTIVE_STATES = ('cancel',)
# Target state a revived work order should land in. On Odoo 17, 'pending' is the
# model default and the functional "Pending / ready to restart" state; the work
# order's own compute then moves it to waiting/ready based on availability.
_WO_RESET_STATE = 'pending'
...
class MrpProduction(models.Model):
    ...
    def action_set_to_draft(self):
        ...
        ``draft`` (F5) and restores work orders to ``pending`` (F8). Runs
    ...
    def _inom_revive_workorders(self):
        """Restore cancelled work orders to the ``pending`` state (F8).

        'pending' is the work order default/restart-ready state; the WO compute
        then settles it into waiting/ready based on component availability. Only
        work orders in ``cancel`` are revived; finished ones are left untouched.
        """
        self.ensure_one()
