...
class UnivHostelAllotment(models.Model):
    ...
    def action_allot(self):
        ...
        for rec in self:
            if not rec.bed_id:
                raise UserError(self.env._("Select a bed to allot."))
            if rec.bed_id.state != "available":
                raise UserError(self.env._("Selected bed is not available."))
    ...
    def _post_fee(self):
        ...
        invoice = self.env["univ.fee.invoice"].create_service_charge(
            ...
            label=self.env._("Hostel allotment: %s", self.hostel_id.name))
        self.fee_invoice_id = invoice.id

    def action_check_in(self):
        for rec in self:
            if rec.state != "allotted":
                raise UserError(self.env._("Allot the bed first."))
    ...
    def action_settle_deposit(self):
        ...
        for rec in self:
            if rec.state != "checkout_req":
                raise UserError(self.env._("Checkout must be requested first."))
            if rec.bed_id:
    ...
    def action_transfer(self):
        for rec in self:
            if rec.state != "approved":
                raise UserError(self.env._("Approve the transfer first."))
            if rec.new_bed_id.state != "available":
                raise UserError(self.env._("New bed is not available."))
