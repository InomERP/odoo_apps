...
publicWidget.registry.WebsiteWalletPaymentOption = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        // ✅ ADD 1: Page restriction (VERY IMPORTANT)
        ...
        console.log("[website_wallet] payment-option widget started on", path);

        this._injected = false;

        // ✅ ADD 2: Safe execution (prevent crash)
        try {
            ...
            console.warn("[website_wallet] safe error:", err);
        }
    },
    ...
    async _tryInject() {
        ...
        // ✅ ADD 3: Safety check (prevent null issue)
        if (!anchor) {
            return;
        }
        ...
        let info;
        try {
            info = await jsonrpc("/shop/wallet/info", {});
        } catch (err) {
        ...
        if (!info.balance || info.balance <= 0) return;
        if (!info.amount_remaining || info.amount_remaining <= 0) return;
