...
class PosAccessRights(models.Model):
    ...
    # ------------------------------------------------------------------
    # POS data-loading hooks (Odoo 19 – pos.load.mixin)
    # ------------------------------------------------------------------
    @api.model
    def _load_pos_data_domain(self, data, *args, **kwargs):
        """Only load the access rule for the current user."""
        return [('user_id', '=', self.env.uid), ('active', '=', True)]

    @api.model
    def _load_pos_data_fields(self, config_id, *args, **kwargs):
        """Fields that must be available in the POS frontend."""
        return [
