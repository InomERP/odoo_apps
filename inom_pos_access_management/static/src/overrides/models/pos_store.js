...
// Odoo 18 path: PosStore lives under `app/store/`.
// (In Odoo 19 it was moved to `app/services/pos_store`.)
...
patch(PosStore.prototype, {
    ...
    _inom_hideNoteButton() {
        ...
        const hideNote = () => {
            // Odoo 18: "Internal Note" button
            document.querySelectorAll("button").forEach((btn) => {
                const text = btn.textContent.trim();
                if (text === "Internal Note" || text === "Note") {
                    btn.style.setProperty("display", "none", "important");
                }
            });
        };
    ...
    _inom_hideRefundButton() {
        const rule = this.accessRule;
        if (!rule?.hide_refund_button) return;
    ...
    _inom_hideInfoButton() {
        const rule = this.accessRule;
        if (!rule?.hide_info_button) return;
   ...
   _inom_hideQuotationButton() {
        const rule = this.accessRule;
        if (!rule?.hide_quotation_button) return;
   ...
   _inom_hideFiscalButton() {
        const rule = this.accessRule;
        if (!rule?.hide_fiscal_button) return;
