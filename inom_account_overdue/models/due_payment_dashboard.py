...
class DuePaymentDashboard(models.Model):
    ...
    def action_open_dashboard(self):
        ...
        if not dashboard:
            dashboard = self.create({
                'name': 'Due Payment Overview'
            })

        return {
            'type': 'ir.actions.act_window',
            'name': 'Dashboard',
            'res_model': 'due.payment.dashboard',
            'view_mode': 'form',
            'res_id': dashboard.id,
            'target': 'current',
        }
    ...
    def _compute_dashboard(self):
        ...
        for rec in self:
            ...
            latest_customer_due = {}
            latest_customer_partner = {}

            for line in customer:
                latest_customer_due[line.invoice_id.id] = line.debit
                latest_customer_partner[line.partner_id.id] = True

            # latest vendor due per bill
            latest_vendor_due = {}
            latest_vendor_partner = {}

            for line in vendor:
                latest_vendor_due[line.invoice_id.id] = line.debit
                latest_vendor_partner[line.partner_id.id] = True
            ...
            # ALERTS (customer + vendor both)
            all_dues = list(latest_customer_due.values()) + list(
                latest_vendor_due.values()
            )

            rec.critical_count = len([
                amt for amt in all_dues
                if amt > 50000
            ])
