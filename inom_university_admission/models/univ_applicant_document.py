...
class UnivApplicantDocument(models.Model):
    ...
    def action_verify(self):
        for record in self:
            if not record.file:
                raise UserError(
                    self.env._("Cannot verify a document with no file attached.")
                )
            record.write(
                {
                    "state": "verified",
            ...
            record.message_post(body=self.env._("Document verified."))
        ...
        for record in self:
            ...
            record.message_post(
                body=self.env._(
                    ...
                    record.reject_reason or self.env._("Not specified"),
                )
            )
    ...
    def action_preview(self):
        ...
        if not self.file:
            raise UserError(
                self.env._("There is no file uploaded to preview yet.")
            )
        return {
            "type": "ir.actions.act_window",
            "name": self.env._("Document Preview"),
        ...
        if was_rejected:
            self.message_post(
                body=self.env._(
                    ...
                    rev=self.version,
                    reason=previous_reason or self.env._("Not specified"),
                )
            )
        else:
            self.message_post(body=self.env._("Document submitted by applicant."))
        return self
