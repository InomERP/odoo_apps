...
class UnivApplicant(models.Model):
    ...
    def action_accept_offer(self):
        ...
        if not self.active_offer_id:
            raise UserError(_("There is no active offer to accept."))
    ...
    def action_view_student(self):
        ...
        if not self.student_id:
            raise UserError(_("No student record is linked yet."))
        return {
            "type": "ir.actions.act_window",
            "name": _("Student"),
    ...
    def action_assign_required_documents(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _("Assign Required Documents"),
    ...
    def _check_can_enrol(self):
        ...
        if self.offer_state != "accepted":
            missing.append(_("an accepted offer"))
        if self.fee_state not in ("paid", "not_required"):
            missing.append(_("admission fee payment"))
        if not self.document_complete:
            missing.append(_("a fully verified document checklist"))
        if self.student_id:
            raise UserError(
                _("Applicant %(name)s is already enrolled.", name=self.name)
            )
        if missing:
            raise UserError(
                _(
