...
class UniversityAdmissionPortal(CustomerPortal):
    ...
    def _compute_next_action(self, applicant):
        ...
        if not applicant.document_complete:
            return request.env._("Upload and complete your document checklist.")
        if applicant.offer_state == "sent":
            return request.env._("Review and accept your provisional offer.")
        if applicant.offer_state == "accepted" and applicant.fee_state == "pending":
            return request.env._("Pay your admission fee to confirm your seat.")
        if applicant.is_enrolled:
            return request.env._("Congratulations! You are enrolled.")
        return request.env._("Your application is under review.")
