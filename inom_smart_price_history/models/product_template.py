...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        """Track Sales Price (list_price) revisions - whether edited
        directly on the product form or synced from a Sale Order line -
        in the same price change log used for Cost/Purchase/Invoice/Bill
        changes, so the Product Price History tab stays complete.
        """
        track = "list_price" in vals and not self.env.context.get(
            "skip_price_log"
        )
        old_prices = {}
        if track:
            for template in self:
                old_prices[template.id] = template.list_price
        res = super().write(vals)
        if track and old_prices:
            log_model = self.env["inom.price.change.log"]
            for template in self:
                old_price = old_prices.get(template.id)
                ...
                if float_compare(template.list_price, old_price,
