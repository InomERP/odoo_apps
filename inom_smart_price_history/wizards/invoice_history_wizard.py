...
class InvoiceHistoryWizard(models.TransientModel):
    ...
    _description = "Customer Invoice Price History"
    ...
    line_ids = fields.One2many(
        comodel_name="invoice.history.wizard.line",
    ...
    def _prepare_history_lines(self, product, partner):
        """Build (0, 0, vals) commands for every customer invoice line that
        matches the given product and the commercial partner of the customer.

        The read is performed with sudo() because a sales user may not hold
        direct accounting access rights, yet must be able to review the price
        history for a product they are already quoting. The query is strictly
        ...
        # Sort by the invoice date in descending order (newest first).
...
class InvoiceHistoryWizardLine(models.TransientModel):
    _name = "invoice.history.wizard.line"
    _description = "Customer Invoice Price History Line"
    _order = "invoice_date desc, id desc"

    wizard_id = fields.Many2one(
        comodel_name="invoice.history.wizard",
        string="Wizard",
        ondelete="cascade",
    )
