...
class HelpdeskTicket(models.Model):
    ...
    _rec_name = 'ticket_number'  # ✅ 1. Yeh add karo

    ticket_number = fields.Char(
        string="Ticket Number",
        copy=False,
        default='New'              # ✅ 2. Yeh add karo
    )
    ...
    def _compute_display_name(self):
        for rec in self:
            rec.display_name = rec.ticket_number or 'New'

    subject = fields.Char(string="Subject")
    description = fields.Text(string="Description")
    priority = fields.Selection([('low', 'Low'),
                                 ('medium', 'Medium'),
                                 ('high', 'High')], default="medium", string="Priority")
    assigned_to = fields.Many2one('res.users', string="Assigned To")
    state = fields.Selection([('new', 'New'),
                              ('in_process', 'In process'),
                              ('resolved', 'Resolved'),
                              ('closed', 'Closed')], default="new", string="Ticket State", group_expand='_group_expand_states')

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('ticket_number') or vals.get('ticket_number') == 'New':
