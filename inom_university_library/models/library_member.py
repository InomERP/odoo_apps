...
class UnivLibraryMember(models.Model):
    ...
    def _compute_name(self):
        for member in self:
            ...
            member.name = rec.display_name if rec else self.env._("Member")
    ...
    def _check_member(self):
        for member in self:
            if member.member_type == "student" and not member.student_id:
                raise ValidationError(self.env._("Select the student."))
            if member.member_type == "faculty" and not member.faculty_id:
                raise ValidationError(self.env._("Select the faculty."))

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("code") or vals["code"] in (self.env._("New"), "New"):
