...
class UnivAttendanceSheet(models.Model):
    ...
    def _compute_name(self):
        for record in self:
            record.name = record.session_id.name or _("Attendance")
    ...
    def action_submit(self):
        for record in self:
            if record.state not in ("draft", "open"):
                raise UserError(_("Sheet is already submitted or locked."))
            if not record.line_ids:
                raise UserError(_("Generate the student list first."))
            record.write({
                "state": "submitted",
    ...
    def action_request_unlock(self):
        ...
        for record in self:
            if record.state != "locked":
                raise UserError(_("Only locked sheets can be reopened."))
