...
class UnivFeeRefundRequest(models.Model):
    ...
    def _check_amount(self):
        for record in self:
            if record.amount <= 0:
                raise UserError(self.env._("Refund amount must be positive."))
            if record.invoice_id and record.amount > record.invoice_id.amount_total:
                raise UserError(
                    self.env._("Refund cannot exceed the invoice total.")
                )
        ...
        for record in self:
            ...
            if record.needs_second_level:
                ...
                record.activity_schedule(
                    ...
                    summary=self.env._("Refund needs registrar approval"),
                )
            else:
    ...
    def action_approve_final(self):
        ...
        for record in self:
            if record.state != "l1_approved":
                raise UserError(
                    self.env._("This request is not awaiting final approval.")
                )
    ...
    def action_post_credit_note(self):
        for record in self:
            if record.state != "approved":
                raise UserError(
                    self.env._("Approve the refund before posting a credit note.")
                )
            if not record.move_id:
                raise UserError(
                    self.env._("The fee invoice has no posted journal entry.")
                )
    ...
    def _create_credit_note(self):
        ...
        credit_vals = {
            ...
            "ref": self.env._("Refund of %s", move.name),
            "journal_id": move.journal_id.id,
            "invoice_line_ids": [
                (0, 0, {
                    "name": self.env._("Fee refund: %s", self.reason or ""),
                    "quantity": 1.0,
    ...
    def action_view_credit_note(self):
        ...
        if not self.credit_note_id:
            raise UserError(self.env._("No credit note posted yet."))
        return {
