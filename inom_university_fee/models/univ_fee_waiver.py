...
class UnivFeeWaiver(models.Model):
    ...
    def _check_amount(self):
        for record in self:
            if record.amount <= 0:
                raise UserError(self.env._("Waiver amount must be positive."))
            if record.invoice_id and record.amount > record.invoice_id.amount_total:
                raise UserError(
                    self.env._("Waiver cannot exceed the invoice total.")
                )
    ...
    def action_approve(self):
        for record in self:
            if record.state not in ("submitted", "under_review"):
                raise UserError(
                    self.env._("Only submitted waivers can be approved.")
                )
    ...
    def action_apply(self):
        for record in self:
            if record.state != "approved":
                raise UserError(
                    self.env._("Approve the waiver before applying it.")
                )
    ...
    def _create_credit_note(self):
        ...
        credit_vals = {
            ...
            "ref": self.env._("Fee waiver: %s", move.name),
            "journal_id": move.journal_id.id,
            "invoice_line_ids": [
                (0, 0, {
                    "name": self.env._("Fee waiver: %s", self.reason or ""),
                    "quantity": 1.0,
