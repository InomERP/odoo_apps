...
class UnivScholarshipAward(models.Model):
    ...
    def action_approve(self):
        for record in self:
            if record.state != "submitted":
                raise UserError(self.env._("Only submitted awards can be approved."))
    ...
    def action_disburse(self):
        ...
        for record in self:
            if record.state != "approved":
                raise UserError(
                    self.env._("Approve the award before disbursing it.")
                )
            ...
            record.message_post(
                body=self.env._("Scholarship of %s disbursed.", record.amount)
            )
    ...
    def _create_adjustment(self):
        ...
        credit_vals = {
            ...
            "ref": self.env._("Scholarship: %s", self.scheme_id.name),
            "journal_id": move.journal_id.id,
            "invoice_line_ids": [
                (0, 0, {
                    "name": self.env._("Scholarship adjustment: %s", self.scheme_id.name),
                    "quantity": 1.0,
    ...
    def action_view_adjustment(self):
        ...
        if not self.credit_note_id:
            raise UserError(self.env._("No adjustment entry posted yet."))
        return {
