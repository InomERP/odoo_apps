...
class UnivFeeInvoice(models.Model):
    ...
    def create_service_charge(self, student, head, amount, label=None,
        ...
        if not student or not head:
            raise UserError(self.env._("A student and a fee head are required."))
        if amount <= 0:
            raise UserError(self.env._("The charge amount must be positive."))
    ...
    def action_view_move(self):
        self.ensure_one()
        if not self.move_id:
            raise UserError(self.env._("No journal entry is linked yet."))
        return {
    ...
    def action_register_payment(self):
        ...
        if not self.move_id or self.move_state != "posted":
            raise UserError(
                self.env._("Post the invoice before registering a payment.")
            )
    ...
    def action_open_installment_plan(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": self.env._("Installment Plan"),
    ...
    def action_request_refund(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": self.env._("Refund Request"),
