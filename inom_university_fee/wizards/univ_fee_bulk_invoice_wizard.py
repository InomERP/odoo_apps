...
class UnivFeeBulkInvoiceWizard(models.TransientModel):
    ...
    def action_generate(self):
        ...
        if structure.state != "confirmed":
            raise UserError(
                self.env._("Confirm the fee structure before generating invoices.")
            )
        students = self.env["univ.student"].search(self._student_domain())
        if not students:
            raise UserError(self.env._("No students match the selected filters."))
        ...
        self.result_summary = self.env._(
        ...
        return {
            ...
            "name": self.env._("Generated Fee Invoices"),
