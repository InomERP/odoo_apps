...
class UnivStudent(models.Model):
    ...
    def _check_can_activate(self):
        ...
        if not self.partner_id:
            missing.append(self.env._("related contact"))
        if not self.program_id:
            missing.append(self.env._("program"))
        if not self.batch_id:
            missing.append(self.env._("batch"))
        if not self.section_id:
            missing.append(self.env._("section"))
        if not self.primary_guardian_id:
            missing.append(self.env._("a primary guardian"))
        if not self.document_complete:
            missing.append(self.env._("a verified document checklist"))
        if missing:
            raise UserError(
                self.env._(
    ...
    def action_enroll(self):
        for record in self:
            if record.state != "draft":
                raise UserError(
                    self.env._("Only applicants can be moved to enrolled.")
                )
    ...
    def action_activate(self):
        for record in self:
            if record.state not in ("draft", "enrolled"):
                raise UserError(
                    self.env._("Only applicants or enrolled students can be activated.")
                )
    ...
    def action_graduate(self):
        for record in self:
            if record.state != "active":
                raise UserError(
                    self.env._("Only active students can be graduated.")
                )
    ...
    def action_drop(self):
        for record in self:
            if record.state in ("graduated",):
                raise UserError(
                    self.env._("Graduated students cannot be dropped.")
                )
