...
class TestSmartCreditLimit(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        group = cls.env.ref(
            'inom_smart_credit_limit.group_smart_credit_manager').sudo()
        membership = 'user_ids' if 'user_ids' in group._fields else 'users'
        group.write({membership: [(4, cls.env.user.id)]})
        ...
        # In 17.0 ``type`` is computed from ``detailed_type``.
