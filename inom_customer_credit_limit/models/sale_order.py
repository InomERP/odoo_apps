...
class SaleOrder(models.Model):
    ...
    @api.depends("partner_id.commercial_partner_id.x_check_credit",
    ...
    def _get_credit_values(self):
        ...
        # self.id is a NewId placeholder for unsaved/onchange records —
        # exclude it from the domain only when it's a real saved record.
        if isinstance(self.id, int):
            domain.append(("id", "!=", self.id))

        other_orders = self.env["sale.order"].search(domain)
        confirmed_so = sum(other_orders.mapped("amount_total"))
        order_amount = self.amount_total
        exposure = receivables + confirmed_so + order_amount
        credit_limit = partner.x_credit_limit
