...
class EdmDocument(models.Model):
    ...
    def _split_by_access(self, operation):
        ...
        if hasattr(self, '_filtered_access'):
            ...
            for record in self:
                try:
                    record._edm_check_access(operation)
                except Exception:
                    continue
                allowed |= record
        return allowed, self - allowed

    def _edm_check_access(self, operation):
        """Check access rights and record rules on this recordset.

        Odoo 18 merged the two checks into ``check_access``; Odoo 17 still
        exposes ``check_access_rights`` and ``check_access_rule`` separately.
        Both shapes are supported so the same code runs on either release.
        """
