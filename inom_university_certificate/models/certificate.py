...
class UnivCertificate(models.Model):
    ...
    def _compute_verify(self):
        ...
        for cert in self:
            if cert.name and cert.name not in (self.env._("Draft"), "Draft"):
    ...
    def action_approve(self):
        for cert in self:
            if cert.state != "request":
                raise UserError(self.env._("Only requested certificates can be approved."))
            cert.state = "approved"
    ...
    def action_generate(self):
        ...
        for cert in self:
            if cert.state != "approved":
                raise UserError(self.env._("Approve the certificate first."))
            if not cert.name or cert.name in (self.env._("Draft"), "Draft"):
    ...
    def action_issue(self):
        for cert in self:
            if cert.state != "generated":
                raise UserError(self.env._("Generate the certificate first."))
    ...
    def action_reissue(self):
        ...
        if self.state != "issued":
            raise UserError(self.env._("Only issued certificates can be re-issued."))
        new = self.copy({
            "version": self.version + 1,
            "state": "approved",
            "name": self.env._("Draft"),
            "body_html": False,
            "signed_hash": False,
            "issue_date": False,
        })
