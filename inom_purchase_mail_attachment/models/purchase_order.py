...
class PurchaseOrder(models.Model):
    ...
    def _get_purchase_mail_product_documents(self):
        """Return product.document records linked to the ordered products that
        are eligible to be attached to purchase emails, based on the company
        mode and each document's own override."""
        self.ensure_one()
        ...
        for doc in documents:
            ...
            if state == 'always':
                eligible |= doc
            elif mode == 'all':  # state == 'default'
                eligible |= doc
        return eligible

    def _get_purchase_mail_vendor_attachments(self):
        """Return vendor (partner) ir.attachment records to attach."""
        self.ensure_one()
    ...
    def _apply_purchase_mail_guards(self, attachments):
        """Apply count and total-size guards. Files are kept in a stable order
        ...
        for att in attachments.sorted(key=lambda a: a.id):
            ...
            if max_bytes and total + size > max_bytes:
                continue  # skip this oversized file, smaller ones may still fit
            kept |= att
            total += size
        return kept

    def _get_purchase_mail_documents(self):
        """Full pipeline: collect product + vendor attachments, apply vendor
        opt-out, dedupe and guards. Returns an ir.attachment recordset."""
        self.ensure_one()
        ...
        # Only real binary files can travel in an email; skip URL attachments.
        attachments = attachments.filtered(lambda a: a.type != 'url')
        return self._apply_purchase_mail_guards(attachments)

    # ------------------------------------------------------------------
    # Official mail template attachment hook
    # ------------------------------------------------------------------
    def _process_attachments_for_template_post(self, mail_template):
        """Hook called by mail.template when generating the email. We append the
        eligible product and vendor documents for each order, but only when the
        template is flagged to carry purchase documents."""
        result = {}
